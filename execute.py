...
_buckets = [(5, 10), (10, 15), (20, 25), (40, 50), (80, 100)]
...
def decode():
    # TEST
    if 0:
        r = requests.get(gConfig['rdany_webhook'])
        if not r:
          print ("r error")
          return
        print (r.text)
        sentense = r.json()["message_text"]
        output_text = "Testing"
        ...
        r = requests.post(gConfig['rdany_webhook'], data=json.dumps(post_data), headers=headers)
        print (r.text)
        return
    # End TEST
    ...
    with tf.Session(config=config) as sess:
        ...
        while 1:
            ...
            if output_text.startswith("_dec_rdany"):
                output_text = output_text[11:]
            now = datetime.today()
            output_text = output_text.replace('<date>', '{}/{}/{}'.format(now.year, now.month, now.day))
            output_text = output_text.replace('<hours>', str(now.hour))
            output_text = output_text.replace('<minutes>', str(now.minute))
            output_text = output_text.replace('<day>', str(now.day))
            output_text = output_text.replace('<month_en>', str(now.month))
            output_text = output_text.replace('<month_es>', str(now.month))
            output_text = output_text.replace('<year>', str(now.year))
